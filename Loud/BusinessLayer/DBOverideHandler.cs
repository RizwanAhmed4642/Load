...
{
    public class DBOverideHandler : IOveride
    {
        private readonly IMapper _mapper;
        private readonly ApplicationDbContext _context;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public DBOverideHandler(IHttpContextAccessor httpContextAccessor, IMapper mapper, ApplicationDbContext context)
        {
            _httpContextAccessor = httpContextAccessor;
            _mapper = mapper;
            _context = context;
        }
        public async Task<ErrorVM> CreateNEditOveride(OverideVM model, string id = "")
        {
            if (id == "")
            {
                try
                {
                    var entity = _mapper.Map<Overide>(model);
                    entity.Created_At = DateTime.Now;
                    entity.Created_By = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                    await _context.Overide.AddAsync(entity);
            ...
            else if (id != "")
            {
                try
                {
                    Overide entity = _mapper.Map<Overide>(model);
                    Overide updatedRecord = await _context.Overide.FindAsync(model.ID);
                    ...
                    else
                    {
                        ...
                        _context.Overide.Update(updatedRecord);
                        await _context.SaveChangesAsync();
                    }
        ...
        public Task<ErrorVM> DeleteOveride(string id)
        {
            try
            {
                if (id != "")
                {
                    _context.Remove(_context.Overide.Single(a => a.ID.ToString() == id));
        ...
        public Task<List<Overide>> GetAllOveride()
        {
            try
            {
                // Retrieve list from the database
                List<Overide> list = _context.Overide.ToList();
            ...
            catch (Exception)
            {
                //The Task.FromResult method creates a Task that represents a precompleted operation.
                //In this case, the operation is returning the list of list.
                return Task.FromResult<List<Overide>>(null);
            }
        }

        public Task<OverideVM> GetSingleOverideWithSearch(string id)
        {
            try
            {
                //Check collection is not empty before using the First method.
                if (_context.Overide.Any(a => a.ID.ToString() == id))
                {
                    Overide mdoel = _context.Overide.First(a => a.ID.ToString() == id);
                    OverideVM data = _mapper.Map<OverideVM>(mdoel);
